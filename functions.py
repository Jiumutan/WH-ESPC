...
class Dataset_CRNN(data.Dataset):
    ...
    def read_images(self, path, selected_folder, use_transform):
        ...
        for i in self.frames:
            ...
            #print(name)
            try:
                if(name[i] != ".ipynb_checkpoints"):
                    character_address = file_dir+"/"+name[i]
            except:
                print(file_dir)
    ...
    def __getitem__(self, index):
        ...
        # print(X.shape)
        return X, y
class Dataset_CLIP(data.Dataset):
    "Characterizes a dataset for PyTorch"
    def __init__(self, data_path, folders, labels, frames, transform=None):
        "Initialization"
        self.data_path = data_path
...
class image_Encoder(nn.Module):
    def __init__(self, fc_hidden1=512, fc_hidden2=512, name="swin_base_patch4_window7_224.ms_in1k", drop_p=0.3, CNN_embed_dim=300, full_number = 1024):
        super(image_Encoder, self).__init__()
        # Transformer 编码器层
        #encoder_layer = nn.TransformerEncoderLayer(d_model, nhead)
        #transformer_encoder = nn.TransformerEncoder(encoder_layer, num_layers=2)
        path = "./"+name+".bin"
        Encoder_model = timm.create_model(name, pretrained=True, num_classes=0,
        ...
        self.transformer_ec = Encoder_model

        # 全连接层
        self.fc1 = nn.Linear(full_number, fc_hidden1)
    ...
    def forward(self, x_3d):
        ...
        for t in range(x_3d.size(1)):
            ...
            with torch.no_grad():
                #print(x_3d[:, t, :, :, :].shape)
                #x = self.resnet(x_3d[:, t, :, :, :])  # ResNet
                #tensor_resized = F.interpolate(x_3d[:, t, :, :, :], size=(256, 256), mode='bilinear', align_corners=False)
                tensor_resized = x_3d[:, t, :, :, :]
                #print(tensor_resized.shape)
                #tensor_resized = self.transforms(tensor_resized).unsqueeze(0)
                #print(tensor_resized.shape)
                tensor_resized =self.transformer_ec(tensor_resized)
                #for i in range(0,tensor_resized.shape[0]):
                    #tensor_resized[i] = self.transforms(tensor_resized[i]).unsqueeze(0)
                #    tensor_resized[i] =self.transformer_ec(tensor_resized[i])
                x = tensor_resized
                #x = x.view(x.size(0), -1)             # flatten output of conv
                #print(x.shape)
...
class DecoderRNN(nn.Module):
    def __init__(self, CNN_embed_dim=300, h_RNN_layers=3, h_RNN=256, h_FC_dim=128, drop_p=0.3, num_classes=50):
        ...
        self.bn3 = nn.BatchNorm1d(h_FC_dim)
        self.drop_p = drop_p
        self.num_classes = num_classes
    ...
    def forward(self, x_RNN):
        ...
        x_3 = self.fc2(x_2)
        x_3 = F.softmax(x_3, dim=1)  # Apply softmax activation function
        #x_3 = F.relu(x_3)
        """ 返回最后一个隐含层和输出层""" 
